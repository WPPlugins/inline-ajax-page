...
function inap_toggle_content(){
	<?php if($inapall['do_effect']){?>
		inap_effects('<?php echo $inapall['do_effect'];?>');
	<?php }else{?>
...
function inap_effects_pick(){
	...
	?>
		if(style1 == 'block'){
			$('#'+where).<?php echo $show_effect[0];?>("<?php echo $show_effect[1];?>");
		}else{
			$('#'+where).<?php echo $hide_effect[0];?>("<?php echo $hide_effect[1];?>");
		}
	<?php }else{?>
		/*inap_effects();*/
