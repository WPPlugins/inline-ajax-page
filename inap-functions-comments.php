...
class INAP_comments{
	...
	function filter($i){
	global $id, $post,$inapall;
		return(ABSPATH . 'wp-content/plugins/inap/inap-comments.php');
	}
	...
	function thread($comment_parent_id=0,$depth=0, $continue=0) {
		...
		if(count($comments[$comment_parent_id+1]) >0){
			if($inapall['hide_child_comments'] == 1 && $depth == 1 && $continue != 1){
	...
	global $inapall,$comment;
		...
		echo "\n\n".'<'.$inapall['comment_tag'].' class= "'.$alt.'" id="comment-'.$comment->comment_ID.'">';

			if($auth && $inapall['comment_tag'] == 'div')
				echo '<div class="'.$auth.'">';
